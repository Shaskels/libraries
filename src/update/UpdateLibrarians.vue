...
export default {
    ...
    methods:{
        ...
        async del(id){
            let result = await axios.delete(`http://localhost:8080/librarian/delete?id=${this.user.id}`)
            console.warn(result);
        ...
        async add(){
            let result = null;
            if (this.isValid())
                result = await axios.put(`http://localhost:8080/librarian/update?surname=${this.user.surname}&name=${this.user.name}&patronymic=${this.user.patronymic}&readingroomsId=${this.readingroom.id}&login=${this.user.login}&password=${this.user.password}&id=${this.$route.params.id}`)
            else{
        ...
        async loadData(){
            ...
            result = await axios.get(`http://localhost:8080/librarian/getById?id=${this.$route.params.id}`)
            console.warn(result)
